...
- **v0.1.6** — YOLO toggle now routes through the existing `/yolo` slash command (`sendMessage("/yolo")`) instead of the REST `DashboardApiClient.yoloToggle()` — the 9119 dashboard never had a `/api/session/yolo` route (only the 8787 webui does), so the 405 was "endpoint not found." Button state reads the authoritative `yolo` flag from `session.info` (dashboard `_session_info()` in tui_gateway/server.py, ORs process `--yolo`, `approvals.mode=off`, per-session flag) in the `SessionInfo` handler — same source as the model chip, updates live from any surface. Removed the unused `yoloStatus()`/`yoloToggle()`. Composer vertical gap (text field → controls row) shrunk 6.dp → 2.dp. (`DashboardChatViewModel.kt`, `DashboardApiClient.kt`, `ChatScreen.kt`, `RpcNotification.kt`.)
- **v0.1.7** — Context token readout left of ring + compress button always visible. (1) Compact `used / max` token gauge (via `formatTokens()`, e.g. `130k / 262k`) renders immediately left of the ctx ring in the composer; ring still clickable → ctx details panel. (2) ctx details panel Compress button now always shown (was gated behind `fraction >= 0.50f || hasUsageData`). (`ChatScreen.kt` composer Row + `ContextDetailsPanel`.) **Release fix:** CI auto-release was failing at `packageRelease` signing. Root cause = missing/incorrect CI keystoress secrets, NOT a JDK issue (same `KeytoolException` on JDK 17 and 21). Fix: set all four keystoress secrets to match `keystore.properties` (`KEYSTORE_BASE64`, `KEYSTORE_PASSWORD`, `KEY_PASSWORD`, `KEY_ALIAS`). The earlier "pin JDK 21" change was a red herring — reverted. GitHub Release v0.1.7 published with APK attached (30,799,620 B). (`app/build.gradle.kts` signing config, `.github/workflows/release.yml`, GitHub Actions secrets.)
